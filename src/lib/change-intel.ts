...
export function summarizeChange(c: ChangeLike): ChangeInsight {
    ...
    if (oldP != null && newP != null && oldP > 0) {
      ...
      if (diff < 0) {
        return {
          category: 'pricing',
          label: 'Price dropped',
          summary: `Rs ${Math.abs(diff).toFixed(0)} down (${pct}%)`,
          tone: 'green',
        };
      }
      if (diff > 0) {
        return {
          ...
          summary: `Rs ${diff.toFixed(0)} up (+${pct}%)`,
          tone: 'red',
        };
      }
    }
  ...
  if (field === 'offers.bsr') {
    ...
    if (oldRank != null && newRank != null) {
      ...
      if (delta > 0) {
        return { category: 'rank', label: 'BSR improved', summary: `#${oldRank.toLocaleString()} -> #${newRank.toLocaleString()}`, tone: 'green' };
      }
      if (delta < 0) {
        return { category: 'rank', label: 'BSR dropped', summary: `#${oldRank.toLocaleString()} -> #${newRank.toLocaleString()}`, tone: 'red' };
      }
    }
  ...
  if (field === 'newReviews') {
    try {
      ...
      return { category: 'reviews', label: 'New review activity', summary: `${count} new review${count === 1 ? '' : 's'} detected`, tone: 'green' };
    } catch {
      return { category: 'reviews', label: 'New review activity', summary: safeText(newValue, 70), tone: 'green' };
    }
  }

  if (field === 'title') {
    return { category: 'content', label: 'Title changed', summary: `${safeText(oldValue, 70)} -> ${safeText(newValue, 70)}`, tone: 'amber' };
  }
  ...
  if (field === 'description') {
    ...
    const suffix = delta === 0 ? 'text rewritten' : `${delta > 0 ? '+' : ''}${delta} chars`;
    return { category: 'content', label: 'Description updated', summary: suffix, tone: 'blue' };
  }

  if (field.startsWith('seo.')) {
    return { category: 'seo', label: `SEO updated (${field.replace('seo.', '')})`, summary: `${safeText(oldValue, 45)} -> ${safeText(newValue, 45)}`, tone: 'blue' };
  }

  if (field.startsWith('offers.')) {
    const name = field.replace('offers.', '');
    return { category: 'offers', label: `Offer updated (${name})`, summary: `${safeText(oldValue, 45)} -> ${safeText(newValue, 45)}`, tone: 'amber' };
  }

  if (field === 'lastReviewAt' || field === 'last_seen_at') {
    return { category: 'listing', label: 'Listing freshness changed', summary: `${safeText(oldValue, 30)} -> ${safeText(newValue, 30)}`, tone: 'gray' };
  }
